...
import { describeChannel } from '../api/ChimeAPI';

const Channels = (props) => {
  const {meetingId, userId, channelArn} = props
    ...
    >
      ...
      <Cell gridArea="side" style={{ height: 'calc(100vh - 3rem)' }}>
        ...
        >
          ...
          <ChannelsWrapper channelArn = {channelArn}/>
        </div>
      </Cell>
      <Cell gridArea="main" style={{ height: 'calc(100vh - 3rem)' }}>
        ...
        {activeChannel.ChannelArn ? (
          ...
          <div className="placeholder">{userId}</div>
        )}
      </Cell>
    </Grid>
  );
